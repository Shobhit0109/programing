def main():
    try :
        arr = []
        n , w = int(input("\n\tEnter total Items : ")) , int(input("\n\tEnter total weight : "))

        [arr.append([i+1,int(input(f"\n\tEnter value of {i+1} item : ")),int(input(f"\n\tEnter weight of {i+1} item : "))]) for i in range(n)]

    except:
        print(end="\n\tWrong value entered default values to be taken")
        n , w = 2, 3
        arr = [[1,1,2],[2,1,1]]
...
if __name__ == "__main__":
    main()
"""
site : https://practice.geeksforgeeks.org/problems/knapsack-with-duplicate-items4201/1
